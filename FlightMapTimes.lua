...
function FlightMapTimes_TakeTaxiNode(id)
  ...
  if (FlightMap.Opts.useTimer and thisNode and thatNode) then
    ...
    local knownTime = map and map[source] and map[source].Flights and map[source].Flights[dest];
    ...
    -- If we are starting to measure a previously unknown route, print a chat message
    if (not knownTime) then
      if FlightMap.Opts and FlightMap.Opts.chatMessages and FLIGHTMAP_MSG_NEW_ROUTE_START then
        DEFAULT_CHAT_FRAME:AddMessage(string.format(FLIGHTMAP_MSG_NEW_ROUTE_START, nodeName), 0.85, 1.0, 0.9);
      end
    end
  else
...
function FlightMapTimes_OnUpdate() 
 ...
 this:SetAlpha(1.0); 
 FlightMapTimesFlash:SetAlpha(0); 
 -- Save measured time if unknown, or if deviates by >= 2s; print message
 local length = GetTime() - this.startTime;
local measured = math.floor(length + 0.5);
local expected = (this.endTime and (this.endTime - this.startTime)) or nil;
local map = FlightMapUtil.getFlightMap();
local originEntry = map and map[this.sourceNode];
local destRaw = map and map[this.destNode] and map[this.destNode].Name;
local destOnly = this.endPoint;
if (not this.endTime) then
  -- New route measured
  if FlightMap.Opts and FlightMap.Opts.chatMessages and FLIGHTMAP_MSG_NEW_ROUTE_TIME then
  ...
  if FlightMapUtil.SaveMeasuredTime and originEntry then
    FlightMapUtil.SaveMeasuredTime(originEntry, this.destNode, destRaw, destOnly, measured);
  end
elseif (expected and math.abs(measured - expected) >= 2) then
  -- Update route if significant deviation
  if FlightMap.Opts and FlightMap.Opts.chatMessages and FLIGHTMAP_MSG_UPDATED_ROUTE_TIME then
 ...
 FlightMapTimesFrame:SetValue(now); 
  ...
  -- Unknown route: show live mm:ss elapsed instead of "(timing)"
  local elapsed = now - this.startTime;
