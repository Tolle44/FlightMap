...
-- === Unknown/Measured time persistence (Vanilla/Turtle-safe) ==============
function FlightMapUtil.SaveMeasuredTime(originEntry, destResolvedKey, destRawName, destOnlyName, seconds)
  if not originEntry or not seconds then return end
  local flights = originEntry.Flights or {}
  originEntry.Flights = flights
  local secs = tonumber(seconds)
  if not secs or secs <= 0 then return end
  if destResolvedKey then flights[destResolvedKey] = secs end
  if destRawName    then flights[destRawName]    = secs end
  if destOnlyName   then flights[destOnlyName]   = secs end
end
